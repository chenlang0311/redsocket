<template>
  <div class="record">
    <button id="btn-start-recording" @click="start">开始录音</button>
    <button id="btn-stop-recording" disabled @click="stop">停止录音</button>
    <hr />
...
<script>
// import "../assets/js/AudioRecorder"
export default {
  name: "Record",
  data() {
    return {
  ...
  methods: {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia && AudioContext) {
      navigator.mediaDevices.getUserMedia({
        ...
        that.log(err.name || err.message,1)
      })
